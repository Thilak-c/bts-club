  ...
  return (
    <div className="min-h-screen flex flex-col bg-[--bg] relative overflow-hidden">
      ...
      <div className="relative z-10 p-6 flex  justify-between">
        <Link href="/" className="w-10 h-10 rounded-full bg-[--card] border border-[--border] flex items-center justify-center hover:border-[--text-dim] transition-colors">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-between px-6 py-8">
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="relative aspect-square w-full max-w-[280px] mx-auto">
            {/* Glow effect */}
            <div className="absolute inset-0 bg-[--primary]/20 blur-[60px] rounded-full scale-75 animate-pulse-soft" />
            ...
            <div className="relative w-full h-full rounded-[32px] border-2 border-[--primary]/40 bg-[--card]/20 backdrop-blur-sm overflow-hidden">
             ...
             {/* / <div className="absolute top-4 left-4 w-6 h-6 border-t-3 border-l-3 border-[--primary] rounded-tl-xl z-10" /> */}
              ...
              {scanning && (
                <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-[--primary] to-transparent animate-scan-smooth shadow-[0_0_20px_rgba(212,175,125,0.6)] z-10" />
              )}
            </div>
          </div>
          ...
          {/* Error message */}
          {error && (
            <div className="mt-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl animate-fade-in">
              <p className="text-red-400 text-[13px] text-center leading-relaxed">
                {error}
              </p>
            </div>
          )}
        ...
        <div className="w-full max-w-sm pb-8">
          <button
            ...
            className="w-full py-4 rounded-2xl bg-[--card] border border-[--border] text-[--text-primary] text-[15px] font-medium hover:border-[--text-dim] transition-all active:scale-[0.98]"
          >
      ...
      {showAnimation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[--bg]/80 backdrop-blur-xl animate-fade-in">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-[400px] h-[400px] bg-[--primary]/10 blur-[120px] rounded-full animate-pulse-soft" />
          </div>

          {/* Main content container */}
          <div className="relative">
            {/* Glassmorphic card with table number */}
            <div className="relative px-12 py-10 rounded-3xl bg-[--card]/40 backdrop-blur-2xl border border-[--border] shadow-2xl opacity-0 animate-spring-in">
              {/* Subtle inner glow */}
              ...
              <div className="relative text-center">
                {/* Success icon */}
                <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-[--primary]/10 backdrop-blur-sm flex items-center justify-center opacity-0 animate-scale-bounce" style={{animationDelay: '0.2s', animationFillMode: 'forwards'}}>
